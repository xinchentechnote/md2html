...
// Windows 仅支持 Win10+（Node 22 基座）；Win7 方案已放弃（Node 18+ 不支持 Win7，
// 14.x 旧基座工具链不再维护，详见 TODO.md 记录）。
...
try {
  ...
  for (const [target, name, isWin] of targets) {
    ...
    execFileSync(
      process.execPath,
      [pkgBin, 'build/md2html.cjs', '--target', target, '--output', out],
