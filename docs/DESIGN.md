...
- 不做公众号 API 直接发布（素材上传/草稿箱），仅做复制粘贴；
- 不做多人协作、云同步、账号体系；
- 不做 Word/PDF 导出；
...
| 图集分组 | 连续的顶层 `image` 节点（≥2 张）合并为 `gallery` 节点，2 张一行；单图保持独立并支持题注（图片下一行 `*斜体*` 段落视为题注，二期） |
...
| 有序列表 | 每项一张浅底圆角卡：主题色圆形序号 + 内容 |
| 无序列表 | 主题色圆点 + 内容，紧凑排布 |
| 任务列表 | ☑/☐ 字符 + 删除线（GFM checked） |
| 表格 | 真实 `<table>` 内联边框；表头主题色浅底；超宽表格降级为逐行卡片（二期） |
...
首发 4 套主题色板方向：摸鱼绿（微信绿 `#07C160`）、红白色系（`#E34D59`）、深海蓝（`#2A6AE9`）、活力橙（`#F76B1C`）。组件结构对齐参考站效果，品牌与文案自定。
