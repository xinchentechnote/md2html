...
export function transform(tree, options = {}) {
  ...
  // 1. 标题卡：HTML 默认包含（复制按钮/publish 时会剥离）；
  //    --no-header / frontmatter header:false 时整体不渲染，h1 留在正文
  ...
  if (options.header !== false) {
    ...
    children.unshift({ type: 'headerCard', title: title || '无标题', date: options.date || formatDate() })
  }
